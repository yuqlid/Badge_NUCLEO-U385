...
static const uint8_t INIT_CMD[] = {
    GC9A01A_INREGEN2, 0, 0xEB, 1, 0x14,                       // ?
    GC9A01A_INREGEN1, 0, GC9A01A_INREGEN2, 0, 0xEB, 1, 0x14,  // ?
    0x84, 1, 0x40,                                            // ?
    0x85, 1, 0xFF,                                            // ?
    0x86, 1, 0xFF,                                            // ?
    0x87, 1, 0xFF,                                            // ?
    ...
    GC9A01A1_POWER2, 1, 0x13, GC9A01A1_POWER3, 1, 0x13, GC9A01A1_POWER4, 1,
    0x22, 0xBE, 1, 0x11,        // ?
    0xE1, 2, 0x10, 0x0E,        // ?
    0xDF, 3, 0x21, 0x0c, 0x02,  // ?
    GC9A01A_GAMMA1, 6, 0x45, 0x09, 0x08, 0x08, 0x26, 0x2A, GC9A01A_GAMMA2, 6,
    0x43, 0x70, 0x72, 0x36, 0x37, 0x6F, GC9A01A_GAMMA3, 6, 0x45, 0x09, 0x08,
    0x08, 0x26, 0x2A, GC9A01A_GAMMA4, 6, 0x43, 0x70, 0x72, 0x36, 0x37, 0x6F,
    0xED, 2, 0x1B, 0x0B,  // ?
    0xAE, 1, 0x77,        // ?
    0xCD, 1, 0x63,        // ?
    ...
    GC9A01A_FRAMERATE, 1, 0x34, 0x62, 12, 0x18, 0x0D, 0x71, 0xED, 0x70,
    0x70,  // ?
    0x18, 0x0F, 0x71, 0xEF, 0x70, 0x70, 0x63, 12, 0x18, 0x11, 0x71, 0xF1, 0x70,
    0x70,  // ?
    0x18, 0x13, 0x71, 0xF3, 0x70, 0x70, 0x64, 7, 0x28, 0x29, 0xF1, 0x01, 0xF1,
    0x00, 0x07,                                                            // ?
    0x66, 10, 0x3C, 0x00, 0xCD, 0x67, 0x45, 0x45, 0x10, 0x00, 0x00, 0x00,  // ?
    0x67, 10, 0x00, 0x3C, 0x00, 0x00, 0x00, 0x01, 0x54, 0x10, 0x32, 0x98,  // ?
    0x74, 7, 0x10, 0x85, 0x80, 0x00, 0x00, 0x4E, 0x00,                     // ?
    0x98, 2, 0x3e, 0x07,                                                   // ?
    GC9A01A_TEON, 0, GC9A01A_INVON, 0, GC9A01A_SLPOUT, 0x80,  // Exit sleep
...
static inline void GC9A01A_set_spi_datasize(GC9A01A *tft, uint32_t data_size) {
  uint32_t crc_length = tft->hspi->Instance->CFG1 & SPI_CFG1_CRCSIZE;
  tft->hspi->Init.DataSize = data_size;
  WRITE_REG(
      tft->hspi->Instance->CFG1,
      (tft->hspi->Init.BaudRatePrescaler | tft->hspi->Init.CRCCalculation |
       crc_length | tft->hspi->Init.FifoThreshold | tft->hspi->Init.DataSize));
}

static inline void GC9A01A_start_spi_transaction(GC9A01A *tft) {
  // AL_GPIO_WritePin(tft->cs_gpio, tft->cs_pin, GPIO_PIN_RESET);
  LL_GPIO_ResetOutputPin(tft->cs_gpio, tft->cs_pin);
}

static inline void GC9A01A_end_spi_transaction(GC9A01A *tft) {
  LL_GPIO_SetOutputPin(tft->cs_gpio, tft->cs_pin);
}

static inline void GC9A01A_transmit_command(GC9A01A *tft, uint8_t command) {
  // set DC pin to LOW, enter command mode
  LL_GPIO_ResetOutputPin(tft->dc_gpio, tft->dc_pin);
  ...
  LL_GPIO_SetOutputPin(tft->dc_gpio, tft->dc_pin);
}

static inline void GC9A01A_transmit_data(GC9A01A *tft, size_t n,
                                         uint16_t *data) {
  GC9A01A_start_spi_transaction(tft);
...
void GC9A01A_init(GC9A01A *tft, SPI_HandleTypeDef *hspi, GPIO_TypeDef *cs_gpio,
                  uint32_t cs_pin, GPIO_TypeDef *dc_gpio, uint32_t dc_pin,
                  GPIO_TypeDef *bl_gpio, uint32_t bl_pin,
                  GPIO_TypeDef *rst_gpio, uint32_t rst_pin);

void GC9A01A_configure_param(GC9A01A *tft, uint8_t command, uint8_t *data,
                             uint8_t n_data);

void GC9A01A_set_addr_window(GC9A01A *tft, uint16_t x1, uint16_t y1, uint16_t w,
                             uint16_t h);
